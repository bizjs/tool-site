...
可直接访问 [https://tool.hstar.vip](https://tool.hstar.vip) 体验！

## 开发

> 推荐使用 pnpm 作为包管理工具
> **需要提前将 .env-example 更名为 .env（复制一份也可，这是 prisma 需要的配置文件）**
...
1. 通过在项目目录执行命令 `docker build -t tool-site:0.0.1 .` 打包 Docker 镜像，其中 0.0.1 为镜像版本号
2. 将镜像推送到镜像仓库，在部署服务器上，通过 `docker pull tool-site:0.0.1` 拉取镜像
3. 通过镜像启动 docker 容器
